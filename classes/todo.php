...
class block_asp_todo {
     ...
     *          object instance
     * @return  The instantiated block_asp_todo object
     * @access  private
     */
     ...
     * @throws  block_asp_invalid_todo_exception if the id is not found
     */
    public function load_by_id($id) {
        global $DB;
        $todo = $DB->get_record('block_asp_step_todos', array('id' => $id));
        if (!$todo) {
            throw new block_asp_invalid_todo_exception(get_string('invalidtodo', 'block_asp'));
        }
    ...
    public function create_todo($todo) {
        ...
        if (!isset($todo->task)) {
            $transaction->rollback(new block_asp_invalid_todo_exception(get_string('tasknotspecified', 'block_asp')));
        }

        // Ensure that a stepid was specified.
        if (!isset($todo->stepid)) {
            $transaction->rollback(new block_asp_invalid_todo_exception(get_string('invalidstepid', 'block_asp')));
        }

        // Ensure that the stepid related to a valid step.
        try {
            new block_asp_step($todo->stepid);
        } catch (Exception $e) {
        ...
        foreach ((array) $todo as $k => $v) {
            if (!in_array($k, $expectedsettings)) {
                $transaction->rollback(new block_asp_invalid_todo_exception(get_string('invalidfield', 'block_asp', $k)));
            }
        }

        // Create the todo.
        $todo->id = $DB->insert_record('block_asp_step_todos', $todo);
     ...
     *          this data set
     * @return  An update block_asp_todo record as returned by
     *          {@link load_by_id}.
     */
    public function update_todo($data) {
        ...
        if (isset($data->stepid) && ($data->stepid != $this->stepid)) {
            $transaction->rollback(new block_asp_invalid_todo_exception(
                    get_string('todocannotchangestepid', 'block_asp')));
        }
        ...
        // Update the record.
        $DB->update_record('block_asp_step_todos', $data);
    ...
    public function delete_todo() {
        ...
        $DB->delete_records('block_asp_todo_done', array('steptodoid' => $this->id));

        // Then remove the actual todo.
        $DB->delete_records('block_asp_step_todos', array('id' => $this->id));
     ...
     *
     * @return  An update block_asp_todo record as returned by {@link load_todo}.
     */
    public function toggle() {
        ...
        // Update the record.
        $DB->update_record('block_asp_step_todos', $update);
    ...
    public static function toggle_task($todoid) {
        $todo = new block_asp_todo($todoid);
        return $todo->toggle();
    }
     ...
     *
     * @return  block_asp_step The step that this task is associated with
     */
