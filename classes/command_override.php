...
/**
 * ASP script command to override a permission.
 *
 * @package   block_asp
 ...
 *
 */
class block_asp_command_override extends block_asp_command {
    public function parse($args, $step, $state = null) {
        ...
        switch ($override) {
            ...
            case "prohibit":
                ...
                break;
            default:
                $data->errors[] = get_string('invalidpermission', 'block_asp');
                return $data;
                break;
        }
        ...
        if (!get_capability_info($cap)) {
            $data->errors[] = get_string('invalidcapability', 'block_asp');
            return $data;
        }
        $data->capability = $cap;
        ...
        if ($this->is_course($step->asp()) && $in != 'course') {
        ...
        switch ($in) {
            case "course":
                if ($state) {
                    ...
                    if ($this->is_course($step->asp())) {
                        // Changing the contextid on the asp's context.
                        $data->contextid = $state->contextid;
                    } else {
                        // Changing the contextid on the asp's parent context.
            ...
            case "activity":
                ...
                break;
            default:
                $data->errors[] = get_string('invalidtarget', 'block_asp');
                break;
        }
