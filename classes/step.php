...
/**
 * Defines the class representing one step in a asp.
 *
 * @package   block_asp
 ...
 * step class
 *
 * Class for handling asp step operations
 *
 * @copyright 2011 Lancaster University Network Services Limited
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 *
 * @property-read int       $id                  The ID of the step
 * @property-read int       $aspid          The ID of the owner asp
...
class block_asp_step {
     ...
     * object   instance
     * @return  The instantiated block_asp_step object
     * @access  private
     */
    private function _load($step) {
        $this->id                   = $step->id;
        $this->aspid           = $step->aspid;
     ...
     * @param stdClass $stepdata raw data, as returned by $DB->get_record('block_asp_steps', ...);
     * @return block_asp_step the corresponding object.
     */
     ...
     * @param   int $stepid The ID of the step to load
     * @return  The instantiated block_asp_step object
     * @throws  block_asp_invalid_step_exception if the id is not found
     */
    public function load_step($stepid) {
        global $DB;
        $step = $DB->get_record('block_asp_steps', array('id' => $stepid));
        if (!$step) {
            throw new block_asp_invalid_step_exception(get_string('noactiveasp', 'block_asp'));
        }
        return $this->_load($step);
    }

    /**
     * Load a step given a aspid and step number
     *
     * @param   int $aspid The ID of the asp this step belongs to
     * @param   int $stepno The number of the step in the asp
     * @return  The instantiated block_asp_step object
     * @throws  block_asp_invalid_step_exception if the id is not found
     */
    public function load_asp_stepno($aspid, $stepno) {
        global $DB;
        $step = $DB->get_record('block_asp_steps',
                array('aspid' => $aspid, 'stepno' => $stepno));
        if (!$step) {
            throw new block_asp_invalid_step_exception(get_string('invalidaspstepno', 'block_asp'));
        }
        return $this->_load($step);
    }
     ...
     * asp and adding to it, or setting it to the first step as
     * appropriate.
     *
     * @param   stdClass $step containing the aspid, name, instructions,
     ...
     * @return  The newly created block_asp_step object
     */
    public function create_step($step, $beforeafter = 0) {
        ...
        // Check that the aspid was specified.
        if (!isset($step->aspid)) {
            $transaction->rollback(new block_asp_invalid_step_exception('invalidaspid', 'block_asp'));
        }

        // Check for a step name.
        if (!isset($step->name) || empty($step->name)) {
            $transaction->rollback(new block_asp_invalid_step_exception('invalidname', 'block_asp'));
        }

        // Check for instructions.
        if (!isset($step->instructions)) {
            $transaction->rollback(new block_asp_invalid_step_exception('invalidinstructions', 'block_asp'));
        }

        // We don't allow a stepid to be specified at create time.
        unset($step->id);

        // This has the effect of checking the specified aspid is valid.
        try {
            $this->asp = new block_asp_asp($step->aspid);
        } catch (Exception $e) {
        ...
        if ($beforeafter !== 0) {
            ...
            $this->asp->renumber_steps($beforeafter, 1);
            $step->stepno = $beforeafter + 1;
        } else {
            // Retrieve the stepno from the final step for this asp.
            $sql = 'SELECT stepno FROM {block_asp_steps} WHERE aspid = ? ORDER BY stepno DESC LIMIT 1';
            $step->stepno = $DB->get_field_sql($sql, array($step->aspid));

            if ($step->stepno) {
                // If there's already a step on this asp, add to that step number.
                $step->stepno++;
            } else {
                // No steps yet for this asp, this is step 1.
                $step->stepno = 1;
            }
        }
        ...
        foreach ((array) $step as $k => $v) {
            if (!in_array($k, $expectedsettings)) {
                $transaction->rollback(new block_asp_invalid_step_exception(get_string('invalidfield', 'block_asp', $k)));
            }
        }
        ...
        if (isset($step->onactivescript)) {
            ...
            if ($result->errors) {
                $transaction->rollback(new block_asp_invalid_command_exception(
                        get_string('invalidscript', 'block_asp', $result->errors[0])));
            }
        }
        ...
        // Create the step.
        $step->id = $DB->insert_record('block_asp_steps', $step);
    ...
    /**
     * Clone an existing step. If a aspid is specified, attach it to
     * that asp instead of the same asp.
     *
     ...
     * @param   int $aspid of the asp to place this step into. If
     *          no aspid is specified, the step is placed within the same
     *          asp as the source
     * @return  The newly created block_asp_step object
     * @static
     */
    public static function clone_step($srcid, $aspid = null) {
        global $DB;
        $transaction = $DB->start_delegated_transaction();

        // Retrieve the source and copy it.
        $src = new block_asp_step($srcid);
        $dst = new stdClass();
        ...
        // If a new aspid was specified, then use it instead.
        if ($aspid) {
            $dst->aspid = $aspid;
        }

        // Create the step.
        $newstep = new block_asp_step();
        $newstep->create_step($dst);

        // Clone the todos.
        foreach ($src->todos() as $todo) {
            block_asp_todo::clone_todo($todo->id, $newstep->id);
        }
        ...
        foreach ($src->roles() as $role) {
            ...
            $doer->roleid = $role->id;
            $DB->insert_record('block_asp_step_doers', $doer);
        }
    ...
    public function delete() {
        ...
        $states = $DB->get_records('block_asp_step_states', array('stepid' => $this->id), null, 'id');
        ...
        // Remove the states.
        $DB->delete_records('block_asp_step_states', array('stepid' => $this->id));

        // Update the atengobacktostep setting for the asp if required.
        $asp           = $this->asp();
        $atendgobacktostep  = $asp->atendgobacktostep;
        if ($atendgobacktostep && $atendgobacktostep > 1 && $this->stepno <= $atendgobacktostep) {
            $asp->atendgobacktostep($atendgobacktostep - 1);
        }

        // Remove the step.
        $DB->delete_records('block_asp_steps', array('id' => $this->id));
     ...
     *          this data set
     * @return  An update block_asp_step record as returned by
     *          {@link load_step}.
     */
    public function update_step($data) {
        ...
        // Check that any specified asp exists.
        if (isset($data->aspid)) {
            try {
                new block_asp_asp($data->aspid);
            } catch (Exception $e) {
                $transaction->rollback($e);
            }
        }
        ...
        foreach ((array) $data as $k => $v) {
            if (!in_array($k, $expectedsettings)) {
                $transaction->rollback(new block_asp_invalid_step_exception(
        ...
        // Update the record.
        $DB->update_record('block_asp_steps', $data);
     ...
     * created, that step must have a asp already assigned. Any value
     * here is overwritten when the step is actually created.
     *
     * @param   integer $aspid The ID of the asp to load for
     * @return  void
     */
    public function set_asp($aspid) {
        if ($this->aspid !== null) {
            throw new block_asp_invalid_step_exception(
                    get_string('aspalreadyset', 'block_asp'));
        }
        $this->aspid = $aspid;
    }
     ...
     * if it is not the only step in the asp.
     *
     * @return  boolean Whether the step may be deleted or not
     * @throws  block_asp_exception If the step is currently in use, and the $throw parameter is true
     */
    public function is_deletable() {
        ...
        // A step may only be removed if there are other steps in the asp.
        $steps = $this->asp()->steps();
        if (count($steps) == 1) {
            return false;
        }
     ...
     *
     * @throws  block_asp_exception If the step is currently in use, or is the only step in the asp
     */
    public function require_deletable() {
        if (!$this->is_deletable()) {
            throw new block_asp_exception(get_string('cannotremoveonlystep', 'block_asp'));
        }
        return true;
    }
     ...
     * @return  The instantiated block_asp_step object
     * @throws  block_asp_not_assigned_exception if no step
    ...
    public function load_active_step($contextid) {
        ...
        $sql = 'SELECT steps.* FROM {block_asp_step_states} state
     ...
     *          - string classname  The name of the asp block class
    ...
    public static function parse_script($script) {
        ...
        foreach ($lines as $line) {
            ...
            // * exist; and
            // * extend block_asp_command, which has all required classes defined.
            $c->classname = 'block_asp_command_' . $c->command;
            if (!class_exists($c->classname)
                    || !is_subclass_of($c->classname, 'block_asp_command')) {
                    $return->errors[] = get_string('invalidcommand', 'block_asp', $c->command);
            } else {
    ...
    public function validate_script($script) {
        ...
        foreach ($commands->commands as $c) {
            $class = block_asp_command::create($c->classname);
     ...
     * @param   string $script   The script to validate
     * @return  boolean          Whether the script is valid
     * @throws  block_asp_invalid_command_exception
     */
    public function require_script_valid($script) {
        ...
        // Check for errors.
        if ($return->errors) {
            throw new block_asp_invalid_command_exception(
                    get_string('invalidscript', 'block_asp', $return->errors[0]));
        }
        return true;
    }
     ...
     *
     * @param   block_asp_step_state $state The step_state to process the script for
     * @param   string $script the script to run.
     * @return  void
     */
    public function process_script(block_asp_step_state $state, $script) {
        global $DB;
        $transaction = $DB->start_delegated_transaction();
        ...
        if ($commands->errors) {
            throw new block_asp_invalid_command_exception(
                    get_string('invalidscript', 'block_asp', $commands->errors[0]));
    ...
    public function get_next_step() {
        ...
        $stepid = $DB->get_field('block_asp_steps', 'id',
                array('aspid' => $this->aspid, 'stepno' => ($this->stepno + 1)));

        if ($stepid) {
            // If there is another step, return that step object.
            return new block_asp_step($stepid);
        }

        if ($stepno = $this->asp()->atendgobacktostep) {
            // If the asp has an atendgobacktostep, load that step.
            $return = new block_asp_step();
            return $return->load_asp_stepno($this->aspid, $stepno);
        }
    ...
    /**
     * Return the asp object that this step belongs to
     *
     * @return block_asp_asp Object for this step's asp
     */
    public function asp() {
        if ($this->asp === null) {
            $this->asp = new block_asp_asp($this->aspid);
        }
        return $this->asp;
    }
     ...
     *
     * @param block_asp_step $with The step to swap with
     * @return  An update block_asp_step record as returned by
     *          {@link load_step}.
     */
    public function swap_step_with(block_asp_step $with) {
        global $DB;
        $transaction = $DB->start_delegated_transaction();
        ...
        // Owing to the unique aspid, stepno constraint, we need to
        // set the stepno to 0 initially.
        $thisstep = new stdClass();
        $thisstep->id       = $this->id;
        $thisstep->stepno   = 0;
        $DB->update_record('block_asp_steps', $thisstep);

        // Then update the step we're swapping with.
        $swapstep = new stdClass();
        $swapstep->id       = $with->id;
        $swapstep->stepno   = $from;
        $DB->update_record('block_asp_steps', $swapstep);
    ...
    public function todos($obsolete = true) {
        ...
        if ($this->todos === null) {
            ...
            $this->todos = $DB->get_records('block_asp_step_todos', $params, 'id');
        }
        return $this->todos;
    }
    ...
    public function toggle_role($roleid) {
        global $DB;
        if ($DB->get_record('block_asp_step_doers', array('stepid' => $this->id, 'roleid' => $roleid))) {
            return $DB->delete_records('block_asp_step_doers', array('roleid' => $roleid, 'stepid' => $this->id));
        } else {
            $role = new stdClass();
            $role->stepid = $this->id;
            $role->roleid = $roleid;
            return $DB->insert_record('block_asp_step_doers', $role);
        }
    }
    ...
    public static function get_autofinish_options($appliesto) {
        ...
        if ($appliesto === 'course') {
            ...
            $options['course;startdate'] = get_string('coursestartdate', 'block_asp');

            // Check whether vl_v_crs_version_pres table exists.
            if (!empty($CFG->hasdataloadtables)) {
                // Here we are using 'db-tablename;relevatfield' as array key.
                $options['vl_v_crs_version_pres;vle_student_open_date'] = get_string('coursestudentopen', 'block_asp');
                $options['vl_v_crs_version_pres;vle_student_close_date'] = get_string('coursestudentclose', 'block_asp');
                $options['vl_v_crs_version_pres;vle_tutor_open_date'] = get_string('coursetutoropen', 'block_asp');
                $options['vl_v_crs_version_pres;vle_tutor_close_date'] = get_string('coursetutorclose', 'block_asp');
            }
        } else if ($appliesto === 'quiz' || $appliesto === 'externalquiz') {
            // The asp applies to the quiz or external quiz.
            ...
            // Here we are using 'db-tablename;relevatfield' as array key.
            $options["$appliesto;timeopen"] = get_string('quizopendate', 'block_asp');
            $options["$appliesto;timeclose"] = get_string('quizclosedate', 'block_asp');
        } else {
    ...
    private static function get_list_of_days($daysbefore, $dayafter) {
        ...
        for ($count = $daysbefore; $count <= $dayafter; $count++) {
            if ($count < 0) {
                ...
                $days[$count * $secondsinday] = get_string($daysbefore, 'block_asp', abs($count));
            }
            if ($count == 0) {
                $days[$count * $secondsinday] = get_string('dayas', 'block_asp');
            }
            if ($count > 0) {
                $daysafter = 'daysafter';
                if ($count === 1) {
                ...
                $days[$count * $secondsinday] = get_string($daysafter, 'block_asp', $count);
            }
        }
        return $days;
    }
