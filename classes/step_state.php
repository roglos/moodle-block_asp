 ...
 * state class
 *
 * Class for handling asp state operations
 *
 * @package    block
 * @subpackage asp
 * @copyright  2011 Lancaster University Network Services Limited
...
class block_asp_step_state {
     ...
     * object instance
     * @return  The instantiated block_asp_step_state object
     * @access private
     */
     ...
     * @return  The instantiated block_asp_step_state object
     * @throws  block_asp_step_states if the id is not found
     */
    public function load_state($stateid) {
        global $DB;
        $state = $DB->get_record('block_asp_step_states', array('id' => $stateid));
        if (!$state) {
            throw new block_asp_exception(get_string('invalidstate', 'block_asp'));
        }
     ...
     * @return  The instantiated block_asp_step_state object or false
     */
     ...
     * @param   int $contextid The ID of the state to load
     * @return  The instantiated block_asp_step_state object
     * @throws  block_asp_not_assigned_exception if the context has no
     *          state assigned
     */
     ...
     * @return  An instantiated block_asp_step_state object
     * @throws block_asp_not_assigned_exception if no state is found for the specified
    ...
    public function load_context_step($contextid, $stepid) {
        ...
        if (!$state) {
            throw new block_asp_not_assigned_exception(get_string('invalidstate', 'block_asp'));
        }
        return $this->_load($state);
    }
     ...
     *
     * @return  block_asp_step The step that this state is associated with
     */
    public function step() {
        if ($this->step === null) {
            $this->step = new block_asp_step($this->stepid);
        }
        return $this->step;
    }
     ...
     * @return  An update block_asp_step_state record as returned by
     *          {@link load_state}.
     */
    ...
    public function change_status($newstatus) {
        ...
        $DB->insert_record('block_asp_state_changes', $change);
    ...
    public function finish_step($newcomment, $newcommentformat) {
        ...
        if ($nextstep = $this->step()->get_next_step()) {
            try {
                ...
                $nextstate = new block_asp_step_state();
                ...
                $newstate->id = $DB->insert_record('block_asp_step_states', $newstate);
                $nextstate = new block_asp_step_state($newstate->id);
            }
    ...
    public function jump_to_step($contextid = null, $newstepid = null) {
        ...
        // Move to the specified step for this asp.
        try {
            // Try and load an existing state to change status for.
            $nextstate = new block_asp_step_state();
    ...
    public function todos() {
        ...
        if ($this->todos === null) {
            $sql = 'SELECT todos.*, done.timestamp, done.userid, done.id AS doneid
                    FROM {block_asp_step_todos} todos
                    LEFT JOIN {block_asp_todo_done} done ON done.steptodoid = todos.id AND done.stepstateid = ?
    ...
    public function todo_toggle($todoid) {
        ...
        $todo = $DB->get_record('block_asp_todo_done', array('stepstateid' => $this->id, 'steptodoid' => $todoid));
        if ($todo) {
            // Remove the current record. There is no past history at present.
            $DB->delete_records('block_asp_todo_done', array('id' => $todo->id));
            $transaction->allow_commit();
            return false;
        } else {
            ...
            $DB->insert_record('block_asp_todo_done', $tododone);
            $transaction->allow_commit();
            return true;
        }
    }
    ...
    public static function state_changes($stateid) {
        ...
        $sql = 'SELECT changes.*, ' . $DB->sql_fullname('u.firstname', 'u.lastname') . ' AS username
                FROM {block_asp_state_changes} changes
     ...
     * @param object $context, the asp context
     */
