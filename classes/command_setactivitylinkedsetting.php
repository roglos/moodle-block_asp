...
/**
 * ASP script commands to set an acitvity setting.
 *
 * @package   block_asp
 ...
 *
 */
class block_asp_command_setactivitylinkedsetting extends block_asp_command {
    ...
    public function parse($args, $step, $state = null) {
        ...
        if (!parent::is_activity($asp)) {
            $data->errors[] = get_string('notanactivity', 'block_asp', 'setactivityvisibility');
            return $data;
        }

        if ($state) {
            $data->cm = get_coursemodule_from_id($asp->appliesto, $state->context()->instanceid);
        }
        ...
        if (!$dbman->table_exists($data->table)) {
            $data->errors[] = get_string('invalidappliestotable', 'block_asp', $asp->appliesto);
            return $data;
        }
        ...
        if (!$dbman->field_exists($data->table, $data->fkcolumn)) {
            $data->errors[] = get_string('invalidactivitysettingcolumn', 'block_asp', $data->fkcolumn);
            return $data;
        }
        ...
        switch (array_shift($line)) {
            case self::CLEAR:
                ...
                if (!empty($line)) {
                    $data->errors[] = get_string('invalidclearmustendcommand', 'block_asp');
                    return $data;
                }
                break;
            ...
            default:
                $data->errors[] = get_string('invalidwordnotclearorset', 'block_asp');
                return $data;
        }
        ...
        if ($data->action == self::SET) {
            ...
            while ($line) {
                ...
                if (!$dbman->field_exists($data->table, $column)) {
                    $data->errors[] = get_string('invalidactivitysettingcolumn', 'block_asp', $column);
                    return $data;
                }

                if (empty($line)) {
                    $data->errors[] = get_string('invalidmissingvalue', 'block_asp', $column);
                    return $data;
                }
