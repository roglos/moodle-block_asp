...
/**
 * Defines the class representing a asp email template.
 *
 * @package    block_asp
...
class block_asp_email {
     ...
     * object   instance
     * @return  The instantiated block_asp_email object
     * @access  private
     */
     ...
     * @return  The instantiated block_asp_email object
     * @throws  block_asp_invalid_email_exception if the id is not found
     */
    public function load_email_id($id) {
        global $DB;
        $email = $DB->get_record('block_asp_emails', array('id' => $id));
        if (!$email) {
            throw new block_asp_invalid_email_exception(get_string('invalidid', 'block_asp'));
        }
     ...
     * @return  The instantiated block_asp_email object or false if the email does not exist
     */
    ...
    public function require_email_shortname($shortname) {
        ...
        if (!$email) {
            throw new block_asp_invalid_email_exception(get_string('invalidemailshortname', 'block_asp', $shortname));
        }
        return $email;
    }
    ...
    public static function load_emails() {
        ...
        $sql = "SELECT emails.*,
            (
                ...
                FROM {block_asp_steps} activescripts
            ...
            (
                ...
                FROM {block_asp_steps} extranotifyscripts
     ...
     * @return  The newly created block_asp_email object
     */
    public function create($email) {
        ...
        if (!isset($email->shortname) || empty($email->shortname)) {
            $transaction->rollback(new block_asp_invalid_email_exception('invalidshortname', 'block_asp'));
        }

        // Check whether this shortname is already in use.
        if ($DB->get_record('block_asp_emails', array('shortname' => $email->shortname))) {
            $transaction->rollback(new block_asp_invalid_email_exception('shortnameinuse', 'block_asp'));
        }

        // Require the message.
        if (!isset($email->message)) {
            $transaction->rollback(new block_asp_invalid_email_exception('invalidmessage', 'block_asp'));
        }

        // Require the subject.
        if (!isset($email->subject)) {
            $transaction->rollback(new block_asp_invalid_email_exception('invalidsubject', 'block_asp'));
        }
        ...
        foreach ((array) $email as $k => $v) {
            if (!in_array($k, $expectedsettings)) {
                $transaction->rollback(new block_asp_invalid_email_exception(
                        get_string('invalidfield', 'block_asp', $k)));
            }
        }

        // Insert the new email.
        $email->id = $DB->insert_record('block_asp_emails', $email);
     ...
     *          in this data set
     * @return  An update block_asp_email record as returned by
     *          {@link load_email_id}.
     */
    public function update($data) {
        ...
        if (isset($data->shortname) &&
                ($id = $DB->get_field('block_asp_emails', 'id', array('shortname' => $data->shortname)))) {
            if ($id != $data->id) {
                $transaction->rollback(new block_asp_invalid_email_exception('shortnameinuse', 'block_asp'));
            }
        }
        ...
        // Update the record.
        $DB->update_record('block_asp_emails', $data);
     ...
     *
     * @throws  block_asp_exception If the email is currently in use
     */
    ...
    public function delete() {
        ...
        $DB->delete_records('block_asp_emails', array('id' => $this->id));
    }
    ...
    private function _used_count($scripts) {
        ...
        foreach ($scripts as $script) {
            $commands = block_asp_step::parse_script($script->script);
