...
/**
 * Base class for asp script commands.
 *
 * @package   block_asp
...
/**
 * The abstract class that each asp command should extend
 *
...
class block_asp_command {
     ...
     * @throws  block_asp_invalid_command_exception If the role does not exist
     */
    public static function require_role_exists($rolename, &$errors) {
        ...
        $errors[] = get_string('invalidrole', 'block_asp', $rolename);
        return false;
    }
    ...
    /**
     * Helper function to determine whether the specified asp applies to an activity
     *
     * @param   object $asp The asp to be tested
     * @return  Boolean
     */
    public static function is_activity($asp) {
        // All asps barring courses are activities.
        return $asp->appliesto != 'course';
    }

    /**
     * Helper function to determine whether the specified context belongs to directly a course
     *
     * @param   object $asp The asp to test
     * @return  Boolean
     */
    public function is_course($asp) {
    ...
    protected function check_next_word_is($requiredword, $actualword, $data) {
        if ($actualword != $requiredword) {
            $data->errors[] = get_string('invalidsyntaxmissingx', 'block_asp', $requiredword);
            return false;
        }
        return true;
    }
