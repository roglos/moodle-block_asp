...
$confirmstr = get_string('deletestepcheck', 'block_asp', $step->name);
$confirmurl = new moodle_url('/blocks/asp/deletestep.php', array('stepid' => $stepid, 'confirm' => 1));
$returnurl  = new moodle_url('/blocks/asp/editsteps.php', array('aspid' => $step->aspid));

// Set page url.
$PAGE->set_url('/blocks/asp/deletestep.php', array('stepid' => $stepid));

// Set the heading and page title.
$title = get_string('confirmstepdeletetitle', 'block_asp', $step->name);
$PAGE->set_title($title);
$PAGE->set_heading($title);

// Add the breadcrumbs.
$PAGE->navbar->add($step->asp()->name, $returnurl);
$PAGE->navbar->add(get_string('deletestep', 'block_asp', $step->name));
