...
if ($taskid) {
    ...
    $returnurl  = new moodle_url('/blocks/asp/editstep.php', array('stepid' => $task->stepid));
    $PAGE->set_title(get_string('edittask', 'block_asp', $task->task));
    $PAGE->set_url('/blocks/asp/edittask.php', array('taskid' => $taskid));
    $data = (object) $task;
} else {
    // Creating a new task. We require the stepid.
    $stepid         = required_param('stepid', PARAM_INT);
    $step           = new block_asp_step($stepid);
    $returnurl      = new moodle_url('/blocks/asp/editstep.php', array('stepid' => $step->id));
    $PAGE->set_title(get_string('createtask', 'block_asp', $step->name));
    $PAGE->set_url('/blocks/asp/edittask.php');
    $data = new stdClass();
    $data->stepid   = $step->id;
}
