...
if ($importform->is_cancelled()) {
    ...
    if (!is_utf8($filecontent)) {
        throw new block_asp_invalid_import_exception(get_string('notutfencoding', 'block_asp'));
    }
    ...
    // Is XML valid?
    if (!$xml) {
        $errors = get_string('xmlloadfailed', 'block_asp');
        foreach (libxml_get_errors() as $error) {
            $errors .= $error->message.', ';
        }
        throw new block_asp_invalid_import_exception($errors);
    }
    // Is it a asp XML?
    if (!($xml->getName() == 'asp')) {
        throw new block_asp_invalid_import_exception(
               get_string('notaasp', 'block_asp'));
    }
    ...
    foreach ($xml->emailtemplates->emailtemplate as $importedtemplate) {
        ...
        if ($email->load_email_shortname($templatex->shortname)) {
            ...
            $extramessage .= get_string('emailtemplateexists', 'block_asp', $templatex->shortname)
                    . ' ';
        } else {
    ...
    // Prepare and validate asp data.
    $aspx = new stdClass();
    $aspx->shortname = clean_and_check_field_validity('shortname', $xml);
    $aspx->name = clean_and_check_field_validity('name', $xml);
    $aspx->description = clean_and_check_field_validity('description', $xml, true, true);
    $descriptionattrs = $xml->description->attributes();
    $aspx->descriptionformat = block_asp_convert_editor_format((string)$descriptionattrs['format']);
    $aspx->appliesto = clean_and_check_field_validity('appliesto', $xml);
    ...
    $asp = $asp->create_asp($aspx, false, true);
    ...
    foreach ($xml->steps->step as $importedstep) {
        ...
        if (in_array($stepno, $steporder)) {
            $transaction->rollback(new block_asp_invalid_import_exception(
                    get_string('notuniquestep', 'block_asp', $stepno)));
        }
        $steporder[] = $stepno;
    }
    ...
    if (!empty($atendgobacktostep)) {
        ...
        if (!array_key_exists($atendgobacktostep, $steporder)) {
            $transaction->rollback(new block_asp_invalid_import_exception(
                    get_string('stepnotexist', 'block_asp', $atendgobacktostep)));
        }
    ...
    foreach ($steporder as $stepkey) {
        ...
        $stepx->instructionsformat = block_asp_convert_editor_format((string)$instructionsattrs['format']);
        ...
        foreach ($importedstep->doer as $doer) {
            ...
            if (!array_key_exists($doer, $roles)) {
                $transaction->rollback(new block_asp_invalid_import_exception(
                        get_string('nosuchrole', 'block_asp', $doer)));
            }
    ...
    if ($atendgobacktostep) {
        // Update asp with new atendgobackto tag.
        $updatedata = new stdClass();
        $updatedata->atendgobacktostep = $atendgobacktostep;
        $asp->update($updatedata);
    }
    ...
    redirect(new moodle_url('/blocks/asp/editsteps.php', array('aspid' => $asp->id)),
            $extramessage . get_string('importsuccess', 'block_asp'), 10);
}
...
function clean_and_check_field_validity($fieldname, $xml, $noempty = true, $html = false) {
    if (!isset($xml->$fieldname)) {
        throw new block_asp_invalid_import_exception(
                get_string('missingfield', 'block_asp', $fieldname));
    }
    ...
    if ($noempty && strlen($field) < 1) {
        throw new block_asp_invalid_import_exception(
                get_string('emptyfield', 'block_asp', $fieldname));
    }
