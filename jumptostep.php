...
require_capability('block/asp:manage', $PAGE->context);

// Grab the current state and the intended state.
$state = new block_asp_step_state();

if (!$state->load_active_state($contextid)) {
    // Jumping back from after the end of the asp, so there is no current
...
// Set the page URL.
$PAGE->set_url('/blocks/asp/jumptostep.php', array('contextid' => $contextid, 'stepid' => $stepid));
$PAGE->set_pagelayout('standard');
$PAGE->set_course($course);

// Set the heading and page title.
$tparams = array('stepname' => $step->name, 'contextname' => $context->get_context_name());
$title = get_string('jumptostepon', 'block_asp', $tparams);
...
$PAGE->navbar->add(get_string('jumpstep', 'block_asp'));
...
$strparams['aspon'] = $context->get_context_name();

$PAGE->set_title(get_string('jumptosteptitle', 'block_asp', $strparams));

$confirmstr = get_string('jumptostepcheck', 'block_asp', $strparams);

// Generate the confirmation button.
$confirmurl = new moodle_url('/blocks/asp/jumptostep.php',
