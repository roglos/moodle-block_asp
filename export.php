...
$wx = new SimpleXMLElement("<?xml version='1.0' encoding='utf-8'?><asp/>");

// Add a version for potential future use.
$wx->addAttribute('version', 2);

// Set the data for the asp itself.
$wx->addChild('shortname',           $asp->shortname);
$wx->addChild('name',                check_output_text($asp->name));
$d = $wx->addChild('description',    check_output_text($asp->description));
$d->addAttribute('format',
        block_asp_editor_format($asp->descriptionformat));
$wx->addChild('appliesto',           $asp->appliesto);
$wx->addChild('atendgobacktostep',   $asp->atendgobacktostep);

// We need to store the various steps.
$sx = $wx->addChild('steps');

// Create a list of the asp's steps.
// We need to store any used templates here.
$templatelist = array();
foreach ($asp->steps() as $step) {
    // Load the object as we'll need to call some of it's functions.
    $step = new block_asp_step($step->id);
    ...
    $i->addAttribute('format',
            block_asp_editor_format($step->instructionsformat));
    ...
    $commands = array();
    $script = block_asp_step::parse_script($step->onactivescript);
    $commands = array_merge($commands, $script->commands);

    $script = block_asp_step::parse_script($step->oncompletescript);
    $commands = array_merge($commands, $script->commands);
    ...
    foreach ($commands as $c) {
        ...
        $class = new block_asp_command_email();
...
// Now add any e-mail templates that this asp currently uses.
...
$filename = preg_replace('/[^a-zA-Z0-9\.-_]/', '_', $asp->shortname) . '.asp.xml';
