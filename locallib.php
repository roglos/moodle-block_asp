...
/**
 * ASP block library code. This file defines some miscellaneous things, and
 * then includes all the asp classes.
 *
 * @package   block_asp
...
require_once($CFG->dirroot . '/blocks/asp/classes/exceptions.php');
require_once($CFG->dirroot . '/blocks/asp/classes/asp.php');
require_once($CFG->dirroot . '/blocks/asp/classes/step.php');
require_once($CFG->dirroot . '/blocks/asp/classes/step_state.php');
require_once($CFG->dirroot . '/blocks/asp/classes/email.php');
require_once($CFG->dirroot . '/blocks/asp/classes/todo.php');
require_once($CFG->dirroot . '/blocks/asp/classes/command.php');
require_once($CFG->dirroot . '/blocks/asp/classes/command_assignrole.php');
require_once($CFG->dirroot . '/blocks/asp/classes/command_email.php');
require_once($CFG->dirroot . '/blocks/asp/classes/command_override.php');
require_once($CFG->dirroot . '/blocks/asp/classes/command_setactivitysetting.php');
require_once($CFG->dirroot . '/blocks/asp/classes/command_setactivityvisibility.php');
require_once($CFG->dirroot . '/blocks/asp/classes/command_setcoursevisibility.php');
require_once($CFG->dirroot . '/blocks/asp/classes/command_setactivitylinkedsetting.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 *
 * @return  array   Containing a list of the all asps
 */
function block_asp_load_asps() {
    global $DB;
    return $DB->get_records('block_asp_asps', null, 'obsolete ASC, appliesto ASC, shortname ASC');
}

/**
 * Return the list of modules that asps may apply to (appliesto)
 *
 ...
 */
function block_asp_appliesto_list() {
 ...
 */
function block_asp_appliesto($appliesto) {
    if ($appliesto == 'course') {
 ...
 */
function block_asp_contextlevel_roles($contextlevel) {
    global $DB;
 ...
 */
function block_asp_editor_options() {
    $options = array();
 ...
 */
function block_asp_editor_format($type) {
    switch ($type) {
        case FORMAT_HTML:
            return get_string('format_html', 'block_asp');
        case FORMAT_PLAIN:
            return get_string('format_plain', 'block_asp');
        default:
            return get_string('format_unknown', 'block_asp');
    }
}
 ...
 */
function block_asp_convert_editor_format($format) {
    $knownformats = array(
        get_string('format_html', 'block_asp')  => FORMAT_HTML,
        get_string('format_plain', 'block_asp') => FORMAT_PLAIN,
    );
    if (isset($knownformats[$format])) {
        return $knownformats[$format];
    } else {
        throw new block_asp_exception(get_string('invalidformat', 'block_asp', $format));
    }
}
 ...
 *
 * That is to say, that ths current user has either the asp:dostep
 ...
 */
function block_asp_can_make_changes($state) {
    global $USER;
    ...
    if (has_capability('block/asp:dostep', $context)) {
 ...
 * @return array
 */
function block_asp_get_active_steps_with_fields_not_null($stepoptions) {
    global $DB;
    ...
    $sql = "SELECT state.id AS stateid, state.stepid, state.state,
                        wf.id AS aspid, wf.appliesto,
                        step.name AS stepname, step.$offsettype, step.$offset,
                        c.id AS courseid, c.shortname AS courseshortname,
                        cm.instance AS moduleid
                FROM {block_asp_step_states} state
                LEFT JOIN {block_asp_steps} step ON step.id = state.stepid
                LEFT JOIN {block_asp_asps} wf ON wf.id = step.aspid
 ...
 * @return int
 */
function block_asp_get_offset_time($courseshortname, $courseid, $moduleid, $offsettype, $offset) {
    global $DB;
 ...
 * @return void
 */
function block_asp_send_extra_notification() {
    $options = array('extranotify', 'extranotifyoffset', 'onextranotifyscript');
    $activesteps = block_asp_get_active_steps_with_fields_not_null($options);
    ...
    foreach ($activesteps as $key => $activestep) {
        $state = new block_asp_step_state($activestep->stateid);
        $notificationtime = block_asp_get_offset_time($activestep->courseshortname,
 ...
 * @return void
 */
function block_asp_autofinish_steps() {
    ...
    foreach ($activesteps as $key => $activestep) {
        $autofinishtime = block_asp_get_offset_time($activestep->courseshortname,
        ...
        if ($autofinishtime < $now) {
            ...
            $newcomment = get_string('finishstepautomatically',  'block_asp',
