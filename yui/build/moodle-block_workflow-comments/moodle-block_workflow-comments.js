YUI.add('moodle-block_asp-comments', function (Y, NAME) {

var COMMENTSNAME = 'blocks_asp_comments',
...
Y.extend(COMMENTS, Y.Base, {
    ...
    show: function (e, finishstep) {
        ...
        if (finishstep) {
            overlay.set('headerContent', M.str.block_asp.finishstep);
            Y.one('.' + CSS.PANEL).one('.' + CSS.SUBMIT + ' input').set('value', M.str.block_asp.finishstep);
            this._formSubmitEvent = Y.one('.' + CSS.SUBMIT + ' input').on('click', this.finishstep, this);
        } else {
            overlay.set('headerContent', M.str.block_asp.editcomments);
            ...
            on: {
                ...
                complete: function(tid, outcome) {
                    ...
                    if (result.response.blockcomments) {
                        ...
                        commentsblock.setContent(M.str.block_asp.nocomments);
                    }
                },
                end: this.removeLoading
            },
            ...
            on: {
                ...
                complete: function(tid, outcome) {
                    ...
                    if (result.response.blockcontent) {
                        // Update content
                        aspblock.setContent(result.response.blockcontent);
                        if (result.response.stateid) {
                            ...
                            this.attachEvents();
                            // reinit todo events
                            M.blocks_asp.init_todolist({"stateid":result.response.stateid});
                        }
                        if (result.response.listasps) {
                            // Last step, avialable asps are listed
                            var select_id = aspblock.one('.singleselect form select').getAttribute('id');
...
M.blocks_asp.init_comments = function(params) {
