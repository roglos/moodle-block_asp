...
/**
 * Server-side script for all ajax request for asp API
 *
 * @package   block_asp
...
$state   = new block_asp_step_state($stateid);
...
$PAGE->set_url('/blocks/asp/ajax.php', array('stateid' => $stateid, 'action' => $action));
...
if (!block_asp_can_make_changes($state)) {
    ...
    throw new block_asp_ajax_exception(get_string('notallowedtodothisstep', 'block_asp'));
}
...
switch ($action) {
    ...
    case 'finishstep':
        ...
        $canview = ($newstate) ? has_capability('block/asp:view', $newstate->context()) : false;

        if ($newstate && ($canview || block_asp_can_make_changes($newstate))) {
            ...
            // Last step has been reached, if permitted retrieve the list of asps.
            $asps = new block_asp_asp();
            $previous = $asps->load_context_asps($state->contextid);
            $canadd = has_capability('block/asp:manage', $state->context());
            $appliesto = $state->step()->asp()->appliesto;
            $addableasps = block_asp_asp::available_asps($appliesto);
            $outcome->response->listasps = $canadd && $addableasps;

            // Display.
            $outcome->response->blockcontent = $renderer->block_display_no_more_steps(
                    $state->contextid, $canadd, $addableasps, $previous);
        }
        break;
    case 'toggletaskdone':
        ...
        break;
    default:
        throw new block_asp_ajax_exception('unknowajaxaction');
}
